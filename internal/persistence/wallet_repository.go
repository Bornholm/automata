...
func (r *WalletRepository) Insert(ctx context.Context, q Querier, e WalletEntry) error {
	_, err := q.ExecContext(ctx, `INSERT INTO wallet_entries
		(org_id, kind, label, amount, created_at, external_ref, price_eur)
		VALUES (?, ?, ?, ?, ?, ?, ?)`,
		e.OrgID, e.Kind, e.Label, e.Amount, formatTenantTime(e.CreatedAt), e.ExternalRef, e.PriceEUR)
	if err != nil {
...
func (r *WalletRepository) List(ctx context.Context, q Querier, orgID string, limit int) ([]WalletEntry, error) {
	query := `SELECT id, org_id, kind, label, amount, created_at, external_ref, price_eur FROM wallet_entries
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.ID, &e.OrgID, &e.Kind, &e.Label, &e.Amount, &createdAt, &e.ExternalRef, &e.PriceEUR); err != nil {
