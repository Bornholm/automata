...
func (r *OrganizationRepository) Insert(ctx context.Context, q Querier, org Organization, ignoreExisting bool) error {
	...
	_, err := q.ExecContext(ctx, verb+` INTO organizations
		(id, display_name, offered, monthly_allowance, unlimited, low_balance_notified_at, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		org.ID, org.DisplayName, offered, org.MonthlyAllowance, unlimited,
...
// Update remplace le nom affiché, le statut offert, l'allocation et le mode
// gratuit sans limite.
func (r *OrganizationRepository) Update(ctx context.Context, q Querier, org Organization) error {
	...
	res, err := q.ExecContext(ctx, `UPDATE organizations
		SET display_name = ?, offered = ?, monthly_allowance = ?, unlimited = ?,
			low_balance_notified_at = ?, updated_at = ?
		WHERE id = ?`,
		org.DisplayName, offered, org.MonthlyAllowance, unlimited,
...
const organizationColumns = `id, display_name, offered, monthly_allowance, unlimited, low_balance_notified_at, created_at, updated_at`

func scanOrganization(scan func(...any) error) (Organization, error) {
	var (
		org                              Organization
		offered, unlimited               int
		notifiedAt, createdAt, updatedAt string
	)
	if err := scan(&org.ID, &org.DisplayName, &offered, &org.MonthlyAllowance, &unlimited, &notifiedAt, &createdAt, &updatedAt); err != nil {
		return Organization{}, err
	}

	org.Offered = offered != 0
	org.Unlimited = unlimited != 0
